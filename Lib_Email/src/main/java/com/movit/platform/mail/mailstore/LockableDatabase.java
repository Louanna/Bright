...
public class LockableDatabase {
    ...
    private class StorageListener implements StorageManager.StorageListener {
        ...
        public void onMount(final String providerId) {
            ...
            try {
                ...
                System.out.println("Unable to open DB on mount");
            }
        }
    }
        ...
        try {
            ...
            try {
                ...
                if (doTransaction) {
                    ...
                    mDb.endTransaction();
                    if (debug) {
                        System.out.println("LockableDatabase: Transaction ended, took " + Long.toString(System.currentTimeMillis() - begin) + "ms / " + new Exception().getStackTrace()[1].toString());
                    }
                }
            }
        } finally {
    ...
    public void switchProvider(final String newProviderId) throws MessagingException {
        if (newProviderId.equals(mStorageProviderId)) {
            System.out.println("LockableDatabase: Ignoring provider switch request as they are equal: " + newProviderId);
            return;
        }
        ...
        try {
            ...
            try {
                try {
                    ...
                    System.out.println("Unable to close DB on local store migration");
                }
    ...
    private void openOrCreateDataspace() throws UnavailableStorageException {
        ...
        try {
            ...
            try {
                ...
                System.out.println("Unable to open DB " + databaseFile + " - removing file and retrying");
                if (databaseFile.exists() && !databaseFile.delete()) {
                    System.out.println("Failed to remove " + databaseFile + " that couldn't be opened");
                }
                doOpenOrCreateDb(databaseFile);
            }
        ...
        try {
            try {
                mDb.close();
                    ...
                    System.out.println( "Exception caught in DB close: " + e.getMessage());
            }
            final StorageManager storageManager = getStorageManager();
            try {
                ...
                if (attachmentDirectory.exists()) {
                    ...
                    if (!attachmentDirectoryWasDeleted) {
                        System.out.println("Attachment directory was not deleted!");
                    }
                }
            } catch (Exception e) {
            ...
            try {
                ...
                System.out.println("LockableDatabase: delete(): Unable to delete backing DB file");
            }
    ...
    private void deleteDatabase(File database) {
        ...
        if (!deleted) {
            System.out.println("LockableDatabase: deleteDatabase(): No files deleted.");
        }
    }
}
