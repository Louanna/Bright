...
public class FileHelper {
        ...
        try {
            if (!file.exists()) {
                ...
                if (!file.setLastModified(System.currentTimeMillis())) {
                    System.out.println("Unable to change last modification date: " + file.getAbsolutePath());
                }
            }
        } catch (Exception e) {
            System.out.println("Unable to touch file: " + file.getAbsolutePath());
        }
    }
    ...
    public static boolean move(final File from, final File to) {
        if (to.exists()) {
            if (!to.delete()) {
                System.out.println( "Unable to delete file: " + to.getAbsolutePath());
            }
        }

        if (!to.getParentFile().mkdirs()) {
            System.out.println("Unable to make directories: " + to.getParentFile().getAbsolutePath());
        }
        ...
        try {
            ...
            if (!from.delete()) {
                System.out.println("Unable to delete file: " + from.getAbsolutePath());
            }
            return true;
        } catch (Exception e) {
            System.out.println("cannot move " + from.getAbsolutePath() + " to " + to.getAbsolutePath());
            return false;
        }
    ...
    public static void moveRecursive(final File fromDir, final File toDir) {
        ...
        if (!fromDir.isDirectory()) {
            if (toDir.exists()) {
                if (!toDir.delete()) {
                    System.out.println( "cannot delete already existing file/directory " + toDir.getAbsolutePath());
                }
            }
            if (!fromDir.renameTo(toDir)) {
                System.out.println("cannot rename " + fromDir.getAbsolutePath() + " to " + toDir.getAbsolutePath() + " - moving instead");
                move(fromDir, toDir);
            }
            return;
        }
        if (!toDir.exists() || !toDir.isDirectory()) {
            if (toDir.exists()) {
                if (!toDir.delete()) {
                    System.out.println( "Unable to delete file: " + toDir.getAbsolutePath());
                }
            }
            if (!toDir.mkdirs()) {
                System.out.println("cannot create directory " + toDir.getAbsolutePath());
            }
        }
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                if (!file.renameTo(target)) {
                    System.out.println("cannot rename " + file.getAbsolutePath() + " to " + target.getAbsolutePath() + " - moving instead");
                    move(file, target);
                }
            }
        }
