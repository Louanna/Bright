...
class MigrationTo51 {
    ...
    public static void db51MigrateMessageFormat(SQLiteDatabase db, MigrationsHelper migrationsHelper) {
        ...
        attachmentDirNew = StorageManager.getInstance(MailboxController.app).getAttachmentDirectory(
        ...
        try {
            System.out.println("migrating " + msgCursor.getCount() + " messages");
    ...
    private static void cleanUpOldAttachmentDirectory(File attachmentDirOld) {
        for (File file : attachmentDirOld.listFiles()) {
            System.out.println("deleting stale attachment file: " + file.getName());
            if (file.exists() && !file.delete()) {
                System.out.println("Failed to delete stale attachement file: " + file.getAbsolutePath());
            }
        }

        System.out.println("deleting old attachment directory");
        if (attachmentDirOld.exists() && !attachmentDirOld.delete()) {
            System.out.println("Failed to delete old attachement directory: " + attachmentDirOld.getAbsolutePath());
        }
    }
