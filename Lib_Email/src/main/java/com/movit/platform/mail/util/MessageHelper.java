...
public class MessageHelper {
    ...
    public void populate(final MessageInfoHolder target,
                         ...
                         Account account) {
        final Contacts contactHelper = MailboxController.showContactName() ? Contacts.getInstance(mContext) : null;
        try {
    ...
    public CharSequence getDisplayName(Account account, Address[] fromAddrs, Address[] toAddrs,String mFolderName) {
        final Contacts contactHelper = MailboxController.showContactName() ? Contacts.getInstance(mContext) : null;

        CharSequence displayName;
//        if (fromAddrs.length > 0 && account.isAnIdentity(fromAddrs[0])) {
//            displayName = toFriendly(toAddrs, contactHelper);
//        } else {
//            displayName = toFriendly(fromAddrs, contactHelper);
//        }

        // FIXME: 16/10/18 收件箱列表只显示发件人
        if(mFolderName.equals(Account.SENT)){
            displayName = toFriendly(toAddrs, contactHelper);
        }else {
            displayName = toFriendly(fromAddrs, contactHelper);
        }
    ...
    public static CharSequence toFriendly(Address address, Contacts contacts) {
        return toFriendly(address, contacts,
                MailboxController.showCorrespondentNames(),
                MailboxController.changeContactNameColor(),
                MailboxController.getContactNameColor());
    }
